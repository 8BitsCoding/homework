...
int ScreenLive::StartEncoder(AVConfig& config)
{
	...
	if (!acodec)
	{
		printf("audio avcodec_find_encoder \n");
		return -1;
	}
	m_aCtx = avcodec_alloc_context3(acodec);
	if (!m_aCtx)
	{
		printf("audio avcodec_alloc_context3 \n");
		return -1;
	}
	m_aCtx->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;   // 渡응뒤寧煉
	m_aCtx->codec_id = AV_CODEC_ID_AAC;    // 긍쯤葵累
	m_aCtx->sample_fmt = AV_SAMPLE_FMT_FLTP; // 긍쯤목駕
	m_aCtx->bit_rate = encoder_config.audio.bitrate;               // bitrate, 箕鍵빈첼취뜩bit
	m_aCtx->sample_rate = encoder_config.audio.samplerate;         // 꽃湳쪽
	m_aCtx->bits_per_raw_sample = 16;               // 覩迦꽃湳貫鑒
	m_aCtx->channels = encoder_config.audio.channels;
	m_aCtx->channel_layout = av_get_default_channel_layout(encoder_config.audio.channels);
	m_aCtx->time_base = { 1, (int)encoder_config.audio.samplerate };    // 珂쇌샘鑒ms槨데貫
	m_aCtx->framerate = { (int)encoder_config.audio.samplerate, 1 };
	m_aCtx->thread_count = 2;               // 긍쯤窟넋鑒

	m_aSwrCtx = swr_alloc_set_opts(m_aSwrCtx,
		m_aCtx->channel_layout,
		m_aCtx->sample_fmt,
		m_aCtx->sample_rate,	//渴놔목駕
		av_get_default_channel_layout(encoder_config.audio.channels),//av_get_default_channel_layout(raw.channels), 
		AV_SAMPLE_FMT_FLTP,//(AVSampleFormat)raw.fmt, 
		encoder_config.audio.samplerate,//raw.sampleRate,
		0, nullptr);
	ret = swr_init(m_aSwrCtx);
	if (ret < 0)
	{
		printf("audio swr_init \n");
		return -1;
	}
	ret = avcodec_open2(m_aCtx, m_aCtx->codec, nullptr);
	if (ret < 0)
	{
		printf("avcodec_open2 \n");
		return -1;
	}
	m_aStream = avformat_new_stream(m_oc, m_aCtx->codec);
	if (!m_aStream)
	{
	...
	if (!vcodec)
	{
		printf("video avcodec_find_encoder \n");
		return -1;
	}
	m_vCtx = avcodec_alloc_context3(vcodec);
	if (!m_vCtx)
	{
		printf("video avcodec_alloc_context3 \n");
		return -1;
	}
	m_vCtx->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;   // 渡응뒤寧煉
	m_vCtx->codec_id = AV_CODEC_ID_H264;    // 긍쯤葵累
	m_vCtx->pix_fmt = AV_PIX_FMT_YUV420P;  // 긍쯤목駕
	m_vCtx->bit_rate = av_config_.bitrate_bps;               // bitrate, 箕鍵빈첼취뜩bit
	m_vCtx->width = 3840;
	m_vCtx->height = 2140;
	m_vCtx->time_base = { 1, (int)av_config_.framerate * 1000 };    // 珂쇌샘鑒ms槨데貫
	m_vCtx->framerate = { (int)av_config_.framerate, 1 };
	m_vCtx->gop_size = 100;                         // 뺌충莉I煉쇌몰
	m_vCtx->max_b_frames = 0;                       // B煉槨0
	m_vCtx->thread_count = 2;               // 긍쯤窟넋鑒
	av_opt_set(m_vCtx->priv_data, "preset", "superfast", 0);

	m_vSwsCtx = sws_getCachedContext(m_vSwsCtx,
		screen_capture_->GetWidth(),//m_vRaw.width, 
		screen_capture_->GetHeight(),//m_vRaw.height, 
		AV_PIX_FMT_BGRA,//(AVPixelFormat)m_vRaw.format,
		screen_capture_->GetWidth(),//m_vCtx->width, 
		screen_capture_->GetHeight(),//m_vCtx->height, 
		AV_PIX_FMT_YUV420P,//m_vCtx->pix_fmt,
		SWS_BICUBIC,
	...
	// ret = avcodec_open2(m_vCtx, m_vCtx->codec, nullptr);
	ret = avcodec_open2(m_vCtx, vcodec, nullptr);
	if (ret < 0)
	{
		std::string s = errStr(ret);
		printf("avcodec_open2 \n");
		return -1;
	}
/*here*/
	m_vStream = avformat_new_stream(m_oc, m_vCtx->codec);
	if (!m_vStream)
	{
		printf("avformat_new_stream \n");
		return -1;
	...
	h264_encoder.setFormatC(m_oc);
	h264_encoder.setMutex(&mtx);

	if (!aac_encoder_.Init(encoder_config)) {
		return -1;
	}
	aac_encoder_.setFormatC(m_oc);
	aac_encoder_.setMutex(&mtx);
		...
		if (nvenc_data_ != nullptr) {
			...
			if (screen_capture_->CaptureFrame(bgra_image, width, height)) {
				...
				if (pkt_ptr != nullptr){
					...
					mtx.lock();
					pkt_ptr.get()->stream_index = m_vStream->index;
					av_interleaved_write_frame(m_oc, pkt_ptr.get());
					mtx.unlock();
				}
			}				
		}		
...
void ScreenLive::EncodeAudio()
{
	...
	{		
		if (audio_capture_.GetSamples() >= (int)frame_samples) {
			...
			if (pkt_ptr) {
				mtx.lock();
				pkt_ptr.get()->stream_index = m_aStream->index;
				av_interleaved_write_frame(m_oc, pkt_ptr.get());
				mtx.unlock();
