...
bool AACEncoder::Init(AVConfig& audio_config)
{
	...
	codec_contex_->sample_rate = av_config_.audio.samplerate;
	codec_contex_->sample_fmt = AV_SAMPLE_FMT_FLTP;
	codec_contex_->channels = av_config_.audio.channels;
	codec_contex_->channel_layout = av_get_default_channel_layout(av_config_.audio.channels);
	codec_contex_->bit_rate = av_config_.audio.bitrate;
...
{
	if (is_initialized_) {
		return codec_contex_->frame_size;
	}
...
{
	if (samples != codec_contex_->frame_size) {
		return nullptr;
	}

	if (!swr_context_) {
		swr_context_ = swr_alloc_set_opts(swr_context_, codec_contex_->channel_layout, codec_contex_->sample_fmt,
			codec_contex_->sample_rate, av_get_default_channel_layout(av_config_.audio.channels),
		...
		if (!pcm_frame_) {
			...
			pcm_frame_->format = codec_contex_->sample_fmt;
			pcm_frame_->channels = codec_contex_->channels;
			pcm_frame_->channel_layout = codec_contex_->channel_layout;
			pcm_frame_->nb_samples = codec_contex_->frame_size;
	...
	int ret = avcodec_send_frame(codec_contex_, pcm_frame_);
	if (ret != 0) {
		return nullptr;
	}
	...
	ret = avcodec_receive_packet(codec_contex_, av_packet_.get());
	...
	mtx->lock();
	//ret = av_interleaved_write_frame(pFormatC, av_packet_.get());
	if (ret < 0) {
		std::cout << "aac write error\n";
	}
	mtx->unlock();
